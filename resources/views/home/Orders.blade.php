                                    ...
                                    @foreach ($details as $d)
                                        <tr>
                                            ...
                                            <td class="d-none d-md-table-cell">Rp.
                                                {{ number_format($d->totalPrice, 0, ',', '.') }}</td>
                                            <td class="d-none d-md-table-cell">
                                                {{ $d->status == 'Ditolak' ? $d->evidencepayment[0]->reason : $d->status }}
                                            </td>
                                            <td><span class="badge badge-success rounded-pill">{{ $d->status }}</span>
                                            </td>
                                            <td>

                                                @if ($d->evidencepayment->isEmpty())
                                                    <a class="btn btn-sm main_btn {{ in_array($d->status, ['Diproses', 'Ditolak']) ? 'disabled' : '' }}"
                                                        href="{{ route('detailsOrder.show', $d->id) }}">
                                                        Ke Pembayaran
                                                    </a>
                                                @else
                                                    Lunas
